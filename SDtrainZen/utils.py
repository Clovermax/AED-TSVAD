...
def instantiate(path: str, args: Optional[dict] = None, initialize: bool = True):
    ...
    module_path = ".".join(splitted_path[:-1])  # eg: "diarizen.models.eend.model_wavlm-base+_conformer"
    class_or_function_name = splitted_path[-1]  # eg: "Model"
    # 这里的importlib.import_module导入了module_path模块, 效果例如: module = importlib.import_module("os")则module.path.join就等价于os.path.join
    module = importlib.import_module(module_path)  # import _module允许在运行时导入一个模块，而不是在代码编写时静态导入
    class_or_function = getattr(module, class_or_function_name)  # getattr是python内置函数, 它接受两个参数：对象 o 和属性名称 name。如果属性存在，则返回该属性的值；如果不存在，则会引发 AttributeError. eg: getattr(os, "path") 等价于 os.path

    if initialize:
        if args:
            return class_or_function(**args)  # 在这里执行模型__init__方法
        else:
