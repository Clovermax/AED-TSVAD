...
from SDtrainZen.logger import TensorboardLogger
from SDtrainZen.optimization import (
...
from SDtrainZen.trainer_utils import TrainerState
from SDtrainZen.utils import prepare_empty_dir, print_env

from SDtrainZen.noam_updater import get_rate
...
class Trainer:
    def __init__(
        ...
        #     shutil.copytree(self.source_code_dir.as_posix(), self.source_code_backup_dir.as_posix(), dirs_exist_ok=True)
        def ignore_symlinks(src, names):
            return [name for name in names if os.path.islink(os.path.join(src, name))]

        if self.accelerator.is_local_main_process:
            shutil.copytree(
                self.source_code_dir.as_posix(), 
                self.source_code_backup_dir.as_posix(), 
                dirs_exist_ok=True,
        ...
        self.plot_lr = self.trainer_config.get("plot_lr", True)
        ...
        # ssl
        self.freeze_ssl = self.trainer_config.get("freeze_ssl", False)
        # freeze
        if self.freeze_ssl:
            has_hubert = hasattr(self.unwrap_model, 'hubert_model') or 'hubert_model' in [name for name, _ in self.unwrap_model.named_modules()]
            has_wavlm = hasattr(self.unwrap_model, 'wavlm_model') or 'wavlm_model' in [name for name, _ in self.unwrap_model.named_modules()]
            if (not has_hubert and not has_wavlm) or (has_hubert and has_wavlm):
                raise ValueError
            if has_hubert:
                logger.info("Freeze HuBERT...")
                self.unwrap_model.freeze_by_name('hubert_model')
            elif has_wavlm:
                logger.info("Freeze WavLM...")
                self.unwrap_model.freeze_by_name('wavlm_model')
    ...
    def _initialize_exp_dirs_and_paths(self, config):
        ...
        Notes:
            ...
            - config_path: /home/xhao/exp/fullsubnet_lr_0.1/config__2023_01_07__17_19_57.yaml
        """
        ...
        # # Save the code
        # shutil.copytree(self.source_code_dir.as_posix(), self.source_code_backup_dir.as_posix(), dirs_exist_ok=True)
        self.config_path = self.exp_dir / f"config__{time_now}.yaml"
    ...
    def create_warmup_scheduler(self, optimizer, scheduler_name, max_steps: int):
        ...
        if scheduler_name == "constant_schedule_with_warmup":
            ...
            return get_cosine_schedule_with_warmup(
                optimizer=optimizer, num_warmup_steps=num_warmup_steps, num_training_steps=max_steps, num_cycles=0.5
            )
        elif scheduler_name == "cosine_with_hard_restarts_schedule_with_warmup":
            return get_cosine_with_hard_restarts_schedule_with_warmup(
                optimizer=optimizer, num_warmup_steps=num_warmup_steps, num_training_steps=max_steps, num_cycles=1
            )
        elif scheduler_name == "polynomial_decay_schedule_with_warmup":
            return get_polynomial_decay_schedule_with_warmup(
                optimizer=optimizer, num_warmup_steps=num_warmup_steps, num_training_steps=max_steps, lr_end=1e-7, power=1.0
            )
        else:
            raise ValueError(
                f"Unknown scheduler name: {scheduler_name}. "
                "Please choose from 'constant_schedule_with_warmup', 'linear_schedule_with_warmup', "
    ...
    def train(self, train_dataloader: DataLoader, validation_dataloader):
        ...
        if self.max_steps > 0:
            ...
            logger.info(f"max_steps={self.max_steps}, update_steps_per_epoch={update_steps_per_epoch}, max_epochs={max_epochs}")
        else:
            max_steps = self.max_epochs * update_steps_per_epoch
            max_epochs = self.max_epochs
            logger.info(f"max_epochs={self.max_epochs}, update_steps_per_epoch={update_steps_per_epoch}, max_steps={max_steps}")
        ...
        if self.warmup_steps > 0 or self.warmup_ratio > 0:
        ...
        # lr_decay_scheduler not implemented
        ...
        for epoch in range(self.state.epochs_trained + 1, max_epochs + 1):
            ...
            # Keep SSL models in eval mode if frozen
            if self.freeze_ssl:
                if hasattr(self.unwrap_model, 'hubert_model'):
                    self.unwrap_model.hubert_model.eval()
                elif hasattr(self.unwrap_model, 'wavlm_model'):
            ...
            for batch_idx, batch in enumerate(dataloader_bar):
                ...
                with self.accelerator.accumulate(self.model):
                    ...
                    # total_DER += loss_dict["DER"]
                    norm_value = loss_dict["Norm"]
                    training_epoch_output.append(loss_dict)

                    dataloader_bar.desc = 'Train[{}]'.format(datetime.now().strftime("%m-%d-%H:%M"))
                    # dataloader_bar.postfix = 'loss={:.4f}, DER={:.4f}'.format(total_loss / (batch_idx + 1), total_DER / (batch_idx + 1))
                    dataloader_bar.postfix = 'loss={:.6f}'.format(total_loss / (batch_idx + 1))

                    # Plot the norm and lr
                    if self.accelerator.is_local_main_process:
                        if self.plot_norm:
                            self.writer.add_scalar("Train_Step/norm", norm_value, self.state.steps_trained)
                        if self.plot_lr:
                            self.writer.add_scalar("Train_Step/lr_small", self.lr_scheduler_small.get_last_lr()[-1], self.state.steps_trained)
                            self.writer.add_scalar("Train_Step/lr_big", self.lr_scheduler_big.get_last_lr()[-1], self.state.steps_trained)
    ...
    def validate(self, dataloader):
        ...
        total_val_loss = 0.0
        total_val_DER = 0.0
        for batch_idx, batch in enumerate(dataloader_bar):  # batch is a dict here
            ...
            total_val_loss += step_output["Loss"]
            total_val_DER += step_output["DER"]

            dataloader_bar.desc = 'Validate[{}]'.format(datetime.now().strftime("%m-%d-%H:%M"))
            dataloader_bar.postfix = 'loss={:.4f}, DER={:.4f}'.format(total_val_loss / (batch_idx + 1), total_val_DER / (batch_idx + 1))
