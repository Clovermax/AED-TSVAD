...
def average_ckpt(ckpt_dir, model, val_metric, avg_ckpt_num, val_mode):
    if 'checkpoints/epoch_' in ckpt_dir or 'checkpoints/best' in ckpt_dir:
    ...
    if val_mode == 'best':
        ...
        val_metric_lst_out = val_metric_lst_sorted[:avg_ckpt_num]
    elif val_mode == 'prev':
        print(f'averaging previous {avg_ckpt_num} checkpoints to the converged moment...')
    ...
    elif val_mode == 'center':
        print(f'averaging center {avg_ckpt_num} checkpoints around the converged moment...')
        val_metric_lst_out = val_metric_lst[
            best_val_metric_idx - avg_ckpt_num // 2 :
            best_val_metric_idx + avg_ckpt_num // 2 + 1
        ]
    else:
        return NotImplementedError
    
    assert len(val_metric_lst_out) == avg_ckpt_num

    pprint(val_metric_lst_out)
