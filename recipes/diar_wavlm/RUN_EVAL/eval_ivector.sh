...
IFS=',' read -r -a eval_set_dir <<< "$eval_set_dir"
...
training_ivector_dir=/data/ssd1/zeyan.song/DATA_${DATASET}/exp_data/NSD-MS2S/metadata/train/ivector_oracle  # substitute with your own path
            ...
            for acn in "${avg_ckpt_num[@]}"; do
                for bt in "${binarize_threshold[@]}"; do
                    ...
                    if [ "$evaluate_on_dev" = "true" ]; then
                        ivector_dir=/data/ssd1/zeyan.song/DATA_${DATASET}/exp_data/NSD-MS2S/metadata/${dset}/ivector_${init_rttm_type}  # substitute with your own path
                        CUDA_VISIBLE_DEVICES=$gpu_id python ${recipe_root}/infer_avg_ivector.py \
                            ...
                            --spk_embedding_txt ${ivector_dir}/ivectors_spk.txt \
                            --training_spk_embedding_txt ${training_ivector_dir}/ivectors_spk.txt \
                    ...
                    for i in "${!eval_set[@]}"; do
                        esubset="${eval_set[$i]}"
                        esubset_dir="${eval_set_dir[$i]}"
                        ivector_dir=/data/ssd1/zeyan.song/${esubset_dir}/exp_data/NSD-MS2S/metadata/${eset}/ivector_${init_rttm_type}  # substitute with your own path
                        CUDA_VISIBLE_DEVICES=$gpu_id python ${recipe_root}/infer_avg_ivector.py \
                            --configuration $config_dir \
